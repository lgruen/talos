...
def test_convert_mt_to_parquet():
    ...
    mt = hl.balding_nichols_model(
        n_populations=1, n_samples=num_samples, n_variants=num_variants)
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        write_ht_to_parquet(ht, os.path.join(
            tmp_dir, 'test'), compression='lz4')
        pq_df = pd.read_parquet(os.path.join(tmp_dir, 'test.parquet'))
